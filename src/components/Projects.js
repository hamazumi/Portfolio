        ...
        <div className="flex flex-wrap -m-4">
          {projects.map((project) => (
            <a
              ...
              <div className="flex relative transition duration-500 ease-out transform hover:scale-105 shadow-2xl drop-shadow-2xl">
                <img
                  alt="gallery"
                  className="absolute inset-0 w-full h-full object-cover object-center z-10 opacity-0 hover:opacity-100"
                  src={project.image}
                />
                <div className="px-8 py-10 relative w-full border-4 border-gray-800 bg-gray-900 shadow-2xl drop-shadow-2xl">
                  <h2 className="tracking-widest text-lg title-font font-medium text-green-400 mb-1">
                    {project.subtitle}
                  </h2>
                  <h1 className="title-font text-2xl font-medium text-white mb-3 animate-pulse">
                    {project.title}
                  </h1>
